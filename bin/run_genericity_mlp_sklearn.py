...
from factslab.utility import ridit, dev_mode_group, get_elmo, load_glove_embedding, features_func
...
if __name__ == "__main__":
    ...
    if args.prot == "arg":
        datafile = home + '/Desktop/protocols/data/noun_raw_data_norm_012919.tsv'
        attributes = ["part", "kind", "abs"]
        attributes_norm = {"Is.Particular.Norm", "Is.Kind.Norm", "Is.Abstract.Norm"}
        attr_map = {"part": "Is.Particular", "kind": "Is.Kind", "abs": "Is.Abstract"}
        attr_conf = {"part": "Part.Confidence", "kind": "Kind.Confidence",
                 "abs": "Abs.Confidence"}
        root_token_col = 'Arg.Token'
        lemma_col = 'Arg.Lemma'
    else:
        datafile = home + '/Desktop/protocols/data/pred_raw_data_norm_012919.tsv'
        attributes = ["part", "hyp", "dyn"]
        attributes_norm = {"Is.Particular.Norm", "Is.Dynamic.Norm", "Is.Hypothetical.Norm"}
        attr_map = {"part": "Is.Particular", "dyn": "Is.Dynamic", "hyp": "Is.Hypothetical"}
        attr_conf = {"part": "Part.Confidence", "dyn": "Dyn.Confidence",
                 "hyp": "Hyp.Confidence"}
        root_token_col = 'Pred.Token'
        lemma_col = 'Pred.Lemma'
    ...
    if args.load_data:
        ...
        # Load the sentences
        sentences = {}
        with open(home + '/Desktop/protocols/data/UD_sentences.tsv', 'r') as f:
            for line in f.readlines():
                structs = line.split('\t')
                sentences[structs[0]] = structs[1].split()
        data['Sentences'] = data['Sentence.ID'].map(lambda x: sentences[x])

        # Load the features
        features = {}
        upos = {}
        with open(home + '/Desktop/protocols/data/features-3.tsv', 'r') as f:
            for line in f.readlines():
                feats = line.split('\t')
...
#                 data_test[resp + ".norm"] = data_test[resp].map(lambda x: 1 if x else 0)
        ...
        tokens = data[root_token_col].tolist()
        tokens = [(x - 1) for x in tokens]
        lemmas = data[lemma_col].tolist()
        sentences = data['Sentences'].tolist()

        data_dev_mean = data_dev.groupby('Unique.ID', as_index=False).apply(lambda x: dev_mode_group(x, attributes_norm, type="regression")).reset_index(drop=True)
        raw_dev_x = data_dev_mean['Structure'].tolist()
        dev_tokens = data_dev_mean[root_token_col].tolist()
        dev_tokens = [(x - 1) for x in dev_tokens]
        dev_lemmas = data_dev_mean[lemma_col].tolist()
        dev_sentences = data_dev_mean['Sentences'].tolist()

        data_test_mean = data_test.groupby('Unique.ID', as_index=False).apply(lambda x: dev_mode_group(x, attributes_norm, type="regression")).reset_index(drop=True)
        raw_test_x = data_test_mean['Structure'].tolist()
        test_tokens = data_test_mean[root_token_col].tolist()
        test_tokens = [(x - 1) for x in test_tokens]
        test_lemmas = data_test_mean[lemma_col].tolist()
        ...
        path = '/data/venkat/pickled_data_' + args.modeltype + '2/' + args.prot
