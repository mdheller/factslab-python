...
def read_data(datafile, attributes, attr_map, attr_conf, regressiontype,
              structures, batch_size, dev_cols):
    '''
    ...
    contexts = [data["Context"].values.tolist()[i:i + batch_size] for i in range(0, len(data["Context"]), batch_size)]
    contexts[-1] = np.append(contexts[-1], contexts[-2][0:len(contexts[-2]) - len(contexts[-1])])
    ...
    dev_contexts = [data_dev_mean["Context"].values.tolist()[i:i + batch_size] for i in range(0, len(data_dev_mean["Context"]), batch_size)]
    dev_contexts[-1] = np.append(dev_contexts[-1], dev_contexts[-2][0:len(dev_contexts[-2]) - len(dev_contexts[-1])])
    ...
    dev = [dev_x, dev_y, dev_tokens, dev_spans, dev_contexts, dev_wts]
    train = [x, y, tokens, spans, contexts, loss_wts]
